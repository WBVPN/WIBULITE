...
function memasang_paket_dasar() {
    ...
    apt upgrade -y
    apt dist-upgrade -y
    DEBIAN_FRONTEND=noninteractive apt install -y at zip pwgen openssl htop netcat-openbsd socat cron bash-completion figlet ruby wondershaper
    DEBIAN_FRONTEND=noninteractive apt install -y lolcat || gem install lolcat
    DEBIAN_FRONTEND=noninteractive apt install -y iptables iptables-persistent
    DEBIAN_FRONTEND=noninteractive apt install -y ntpdate chrony
...
memasang_notifikasi_bot() {
  ...
  local CITY=$(curl -s --max-time 5 ipinfo.io/city)
  local ISP=$(curl -s --max-time 5 ipinfo.io/org | cut -d " " -f 2-10)
...
"
  ...
  local INLINE_KEYBOARD='{"inline_keyboard":[[{"text":"Telegram","url":"https://t.me/wibuvpn"}]]}'
...
function memasang_xray() {
    ...
    clear
    curl -s --max-time 5 ipinfo.io/city >> /etc/xray/city
    curl -s --max-time 5 ipinfo.io/org | cut -d " " -f 2-10 >> /etc/xray/isp
    print_install "Memasang Konfigurasi Paket"
    wget -T 5 -O /etc/nginx/conf.d/xray.conf "${REPO}config/xray.conf" >/dev/null 2>&1
...
function memasang_password_ssh(){
    ...
    wget -T 5 -O /etc/pam.d/common-password "${REPO}files/password"
...
wget -T 5 -q -O /etc/ssh/sshd_config "${REPO}files/sshd" >/dev/null 2>&1
...
type = drive
scope = drive
token = {\"access_token\":\"ya29.DUMMY_TOKEN_FOR_GDRIVE\",\"token_type\":\"Bearer\",\"refresh_token\":\"1//0gDUMMY_REFRESH_TOKEN\",\"expiry\":\"2022-10-25T12:15:50.5813586+08:00\"}"
...
memasang_pencadangan() {
  ...
  export DEBIAN_FRONTEND=noninteractive
  DEBIAN_FRONTEND=noninteractive apt install rclone -y
...
function memasang_sysctl_tuning(){
    clear
    print_install "Optimasi Kernel untuk Koneksi Tinggi"
    cat >> /etc/sysctl.conf <<SYSEOF

# === WIBULITE Connection Optimization ===
...
function memasang_bbr_hybla(){
  ...
  DEBIAN_FRONTEND=noninteractive apt install -y ethtool net-tools haveged htop iftop
...
function memasang_fail2ban(){
    ...
    for file in ddos.conf LICENSE ignore.ip.list ddos.sh; do
        wget -T 5 -q -O "/usr/local/ddos/$file" "http://www.inetbase.com/scripts/ddos/$file" || \
...
wget -T 5 -q -O /usr/local/share/xray/geosite.dat "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geosite.dat" >/dev/null 2>&1
wget -T 5 -q -O /usr/local/share/xray/geoip.dat "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geoip.dat" >/dev/null 2>&1
...
wget -T 5 -O /usr/bin/badvpn-udpgw "${REPO}files/newudpgw"
...
systemctl daemon-reload
systemctl restart nginx ssh dropbear ws-stunnel fail2ban vnstat cron atd server-sldns udp-custom noobzvpns haproxy
...
cat >/etc/cron.d/xp_all <<-END
    ...
    # Xray log rotation - truncate every 5 minutes to prevent memory bloat
    echo "*/5 * * * * root [ -f /var/log/xray/access.log ] && truncate -s 0 /var/log/xray/access.log" >/etc/cron.d/log.xray

    # Batched xray restart - check flag file every minute, restart only once
    echo "*/1 * * * * root [ -f /tmp/.xray-need-restart ] && rm -f /tmp/.xray-need-restart && systemctl restart xray >/dev/null 2>&1" >/etc/cron.d/xray.restart
...
wget -T 5 -q -O /etc/default/dropbear "${REPO}config/dropbear.conf"
chmod +x /etc/default/dropbear
wget -T 5 -q -O /etc/banner-ssh.txt "${REPO}files/issue.net"
...
function memasang_sshws(){
    ...
    wget -T 5 -O /usr/local/bin/ws-stunnel ${REPO}files/ws-stunnel
    wget -T 5 -O /usr/bin/tun.conf "${REPO}config/tun.conf" >/dev/null 2>&1
    chmod +x /usr/local/bin/ws-stunnel
    wget -T 5 -O /etc/systemd/system/ws-stunnel.service ${REPO}files/ws-stunnel.service && chmod +x /etc/systemd/system/ws-stunnel.service
    systemctl daemon-reload
...
DEBIAN_FRONTEND=noninteractive apt install -y python3 python3-dnslib net-tools
DEBIAN_FRONTEND=noninteractive apt install ncurses-utils -y
DEBIAN_FRONTEND=noninteractive apt install dnsutils -y
DEBIAN_FRONTEND=noninteractive apt install ncurses-utils -y
DEBIAN_FRONTEND=noninteractive apt install -y whois
DEBIAN_FRONTEND=noninteractive apt install -y sudo gnutls-bin
DEBIAN_FRONTEND=noninteractive apt install -y debconf-utils
service cron reload
...
wget -T 5 -q -O /etc/slowdns/server.key "${REPO}slowdns/server.key"
wget -T 5 -q -O /etc/slowdns/server.pub "${REPO}slowdns/server.pub"
wget -T 5 -q -O /etc/slowdns/sldns-server "${REPO}slowdns/sldns-server"
cd
...
[Unit]
Description=UDP CUSTOM BY WIBUVPN
[Service]
User=root
Type=simple
...
sudo env DEBIAN_FRONTEND=noninteractive apt install haproxy -y
...
frontend ssl
    ...
    use_backend GRUP_WIBUVPN if up-to
    use_backend WIBUVPN if chk-02_up chk-02_ws
    use_backend WIBUVPN if { path_reg -i ^\/(.*) }
    use_backend BOT_WIBUVPN if this_payload
    default_backend CHANNEL_WIBUVPN
